...
\begin{titlepage}
		...
		\today
		\vfill
		\includegraphics[width=15cm]{1Dfsinx.jpg}
		\vfill
...
\begin{equation}
	\begin{cases}
		-\frac{k}{\mu}\triangle\vec{p}=-\sum_{p=1}^{n_{well}}Q_p\delta(x-x_p)=0,\, \, (x,y) \, \epsilon \, \Omega\\
...
\begin{equation}
	S^{be_l}_{ij} = \int_{b_l} K\phi_i \phi_j dx
\end{equation}

\begin{equation}
	f^{be_l}_i = Kp^H\int_{b_l}\phi_i dx
\end{equation}
...
Galerkin equations
\begin{equation}
\phi(\vec{x})=\phi(\vec{x})_i \,\,\,v_x\approx v^{(n)}_x = \sum_{j=1}^{n}c_j\phi(\vec{x})_j
\end{equation}
