...
\begin{equation}
	\nabla\cdot\vec{v}=-\sum_{p=1}^{n_{well}}Q_p\delta(\phi(\vec{x}-\phi(\vec{x}_p)=0,\quad (x,y) 	\in\Omega 
\end{equation}

Where $Q_p$ denotes the water extraction rate by well k, which is located at $x_p$. Here $x$ equals $(x;y)$, the spatial coordinates.  We use the convention $\vec{x} = (x; y) $ to represent the spatial coordinates. The dirac Delta Distribution is characterized by equation(2.3).

\begin{equation}
	\begin{cases} 
		\delta(\vec{x}) = 0,\quad  \vec{x} \neq 0 \\ \int_{\Omega}\delta(\vec{x})d\Omega = 1, \quad $  where $\Omega $  contains the origin.$
	\end{cases} 
\end{equation}
...
\begin{equation}
	\vec{v}\cdot\vec{n}=K(p-p^H), \, (x,y)\, \in\,  \partial\Omega
\end{equation}
...
We also divide the boundary $\partial\Omega$ into four parts described by the side of the square domain $\Omega$. $\partial\Omega_1$ is the part with $x = -1$, $\partial\Omega_2$ is the part with $y = 1$, $\partial\Omega_3$ is the part with $x = 1$, $\partial\Omega_4$ is the part with $y = -1$.
\newpage
...
$ p \in \{1, \dots ,5\}$ and for $p = 6$ we have $ x_6 = 0 $ and $ y_6 = 0 $.
...
\text{BVP  }
	\begin{cases}
		-\frac{k}{\mu}\triangle\vec{p}=-\sum\limits_{p=1}^{n_{\text{well}}}Q_p\delta(\vec{x}-\vec{x}_p)=0,\, \, \quad (x,y) \, \in \, \Omega\\
		-\frac{k}{\mu}\nabla\vec{p}\cdot\vec{n}=-\frac{k}{\mu}\frac{dp}{dn} =K(p-p^H), \, \quad (x,y)\,  \in  \, \partial\Omega
	\end{cases}
\end{equation}
...
\begin{equation}
	\int_{\delta\Omega}\phi(\vec{x})K(p-p^H) d\tau+\int_{\Omega}\frac{k}{\mu}\nabla\phi(\vec{x})\cdot\nabla p d\Omega= -\sum_{p=1}^{n_{well}}\phi(\vec{x}_p) Q_p
\end{equation}	
...
(WF): \begin{equation}
		\begin{cases} 
			...
			\int_{\delta\Omega}\phi(\vec{x})Kp d\tau+\int_{\Omega}\frac{k}{\mu}\nabla\phi(\vec{x})\cdot\nabla p d\Omega= -\sum_{p=1}^{n_{well}}\phi(\vec{x}_p) Q_p +\int_{\delta\Omega}\phi(\vec{x})Kp^Hd\tau\\ \forall\phi $ $ \epsilon\sum 
		\end{cases}\  
	\end{equation}
...
\begin{equation}
	\sum_{j=1}^{n}c_i\int_{\partial\Omega}\phi_i K\phi_j d\tau + \int_{\Omega}\frac{k}{\mu}\nabla\phi(x)\cdot\nabla \phi_j d\Omega= -\sum_{p=1}^{n_{well}}\phi(x_p) Q_p +\int_{\partial\Omega}\phi_i Kp^Hd\tau
\end{equation}
...
\begin{equation}\label{f_i_ek}
	f^{e_k}_i =  -\sum_{p=1}^{n_{well}}\phi_i(\vec{x}_p) Q_p
\end{equation}
...
\begin{equation}
	S^{be_l}_{ij} = \int_{be_l} K\phi_i \phi_j dx = K\frac{\lvert be_l\rvert}{6}(1+\delta_{ij})
\end{equation}

\begin{equation}
	f^{be_l}_i = Kp^H\int_{be_l}\phi_i dx = K p^H \frac{\lvert be_l\rvert}{2}
\end{equation}
Where $\delta_{ij}$ is the Kronicker delta.

\section{Wells within an internal element}

To solve the BVP in 2D, one of the aspects that need to be determined is whether each internal element contains a well. This is done by determining whether well with index $p$ and position $x_p$ is contained within element $e_k$ with vertices $x_{k1}$, $x_{k2}$ and $x_{k3}$. This is done according the following criterion:
...
Therefore, the method hinted at in the assignment was used instead. We check whether the linear basis functions $\phi_{k1}$, $\phi_{k2}$ and $\phi_{k3}$ all have values in the interval $ [0,1] $ at position $\vec{x}_p$. If this is true, then the well at $\vec{x}_p$ is within the triangular element $e_k$.

If we find and element that does contain a well, we see in equation \eqref{f_i_ek} that we must find $\phi_i(\vec{x_p})$ for $i = \{k1, k2, k3\}$. The following code does this by determining $\alpha_i$, $\beta_i$, and $\gamma_i $ and subsequently filling $\vec{x}_p$ into the found $\phi_i(\vec{x})$.
\begin{lstlisting}
...
Similar as with the 1D BVP, code is written in order to generate a mesh, element matrix, element vector, boundary element matrix and boundary element vector. These codes can be found in appendix B.1 through B.6. The scripts have been written such that they can be used to find both the pressure field and the velocity field.
...
In this part we use Darcy's law to compute the speed in both directions.

%\begin{equation}
% M\vec{v_x}=C_x\vec{p},\,\,\, M\vec{v_y}=C_y\vec{p}
%\end{equation}

In order to find $v_x$ and $v_y$ we first rewrite equation(2.20).
...
\begin{equation}
v_x=-\frac{k}{\mu}\frac{dp}{dx}
\end{equation}


\begin{equation}
v_y=-\frac{k}{\mu}\frac{dp}{dy}
\end{equation}
...
In order to find the weak form we, again, use the test function $\phi$ and integrate over the domain $\Omega$. Here we go through the derivation for $v_x$, the steps for deriving $v_y$ are similar.

\begin{equation}
\int\limits_{\Omega}\phi v_x d\Omega=-\frac{k}{\mu}\int\limits_{\Omega}\phi\frac{dp}{dx}d\Omega
\end{equation}

Partial integration is applied on the right side term.
\begin{equation}
\int\limits_{\Omega}\phi v_x d\Omega=  -\frac{k}{\mu}\{\int\limits_{\Omega}\frac{d}{dx}(\phi p)-p\frac{d\phi}{x}d\Omega\}
\end{equation}

Rewriting the integral:

\begin{equation}
 \int\limits_{\Omega}-\frac{k}{\mu}\frac{d\phi p}{dx}dxdy=\int\limits_{-1}^{1}-\frac{k}{\mu}[\phi p]^{1}_{-1}dy
\end{equation}

The surface integral turns into a set of line integrals along parts of the boundary $\partial\Omega$.
\begin{equation}
\int\limits_{\Omega}-\frac{k}{\mu}\frac{d\phi p}{dx}dxdy=  \int\limits_{-1}^{1}-\frac{k}{\mu}(\phi(x=1,y)p(x=1,y))+\frac{k}{\mu}(\phi(x=-1)p(x=-1,y))dy
\end{equation}

Rewriting the equation:

\begin{equation}
\int\limits_{\Omega}-\frac{k}{\mu}\frac{d\phi p}{dx}dxdy =\int\limits_{\partial\Omega_3}-\frac{k}{\mu}\phi pd\tau+\int\limits_{\partial\Omega_1}\frac{k}{\mu}\phi pd\tau
\end{equation}

Inserting this into equation(2.29) we get the following equation.

\begin{equation}
	\int\limits_{\Omega}\phi v_x = \frac{k}{\mu}\{\int\limits_{\partial\Omega_3}-\phi pd\tau+\int\limits_{\partial\Omega_1}\phi pd\tau+\int\limits_{\Omega}p\frac{\phi_i}{dx}d\Omega\}
\end{equation}

We use the boundary conditions to rewrite $p$ in the two boundary integrals.
...
We find the following weakform.
...
To find the system of equations we fill in  
$\phi(\vec{x})=\phi_i(\vec{x})= \alpha_i+\beta_i x+\gamma_i y$ and  $v_x\approx \sum\limits_{j=1}^{n}c_j\phi_j(\vec{x})$.

\begin{equation}
S_{ij}=\sum_{j=1}^{n}c_j
\{\int\limits_{\Omega}\phi_i\phi_j d\Omega +\int\limits_{\partial\Omega_3}-\frac{k}{\mu}\frac{1}{k}\phi_i\phi_j d\tau + \int\limits_{\partial\Omega_1}-\frac{k}{\mu}\frac{1}{k}\phi_i\phi_j d\tau \}
\end{equation}

\begin{equation}
f_i = 
\int\limits_{\partial\Omega_3}-\frac{k}{\mu}\phi_i p^H d\tau +\int\limits_{\partial\Omega_1}\frac{k}{\mu}\phi_i p^H d\tau + \int\limits_{\Omega}\frac{k}{\mu}p\frac{d\phi_i}{dx}d\Omega
\end{equation}

We separate contributions to both $S_{ij}$ and $f_i$ from boundary and internal elements into $S_{ij}^{be_l}$, $S_{ij}^{e_k}$, $f_i^{be_l}$ and $f_i^{e_k}$ such that:
...
Applying Newton-Cotes theorem and Holand-Bell theorem we find new expressions for the (boundary) element-matrix and -vector.
...
Since we have previously calculated the pressure field $p$ we have all the necessary information to calculate $v_x$ (and similarly $v_y$).
...
Setting $K=0$ means the transfer coefficient of the water between the boundary and its surroundings is zero.
